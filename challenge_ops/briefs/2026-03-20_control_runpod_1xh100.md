...
- What is being tested: run one same-workflow remote `1xH100` control rerun on a Runpod `1xH100` pod using the standard `control` wrapper path, unchanged `sp1024` dataset, and unchanged tokenizer.
- Why now: the latest remote `lr_warmdown` rerun regressed on the same nominal scope, so the cheapest discriminating next step is to measure whether the remote workflow itself drifts away from the Runpod `1xH100` control anchor.

## Anchor And Novelty Check

- Closest prior idea or run: `experiments/ledger.csv` row `ablate_control_1xh100_1024`
- Anchor metric and scope: legacy baseline alias `baseline_sp1024_h100_local_20260319 = 1.32321114` and control `ablate_control_1xh100_1024 = 1.32157507`, both representing the Runpod `1xH100` control anchor on `1xH100-surrogate`
...
- Hardware target: `Runpod 1xH100 pod` (H100 SXM path)
...
- Success threshold: final parsed metrics present and remote `val_bpb` lands close to the Runpod `1xH100` control anchor, making the recent `lr_warmdown` regression look idea-specific rather than infrastructure-driven.
...
- Extra context: if the remote control rerun also drifts materially worse than the Runpod `1xH100` control anchor, stop recommending more paid ablations and write an infra-variance diagnosis instead.
